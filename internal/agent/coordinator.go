...
// configured pair.
//
// A pair that came from the client is flagged requested, which is what
// keeps a one-shot `crush run -m` from rewriting the project's saved
// selection (see persistSessionModels).
...
func (c *coordinator) buildTools(ctx context.Context, agent config.Agent, isSubAgent bool) ([]fantasy.AgentTool, error) {
	...
	// The new_session tool is not available to sub-agents: the handoff it
	// triggers is handled by the TUI, which never happens for a sub-agent
	// run. Context status is only injected in llm compaction mode, so the
	// description must match. Auto-summarize mirrors compactionFlags:
	// always on in llm mode, otherwise honoring disable_auto_summarize.
	//
	// Like the question tool below, it is withheld from non-interactive
	// turns per turn in sessionAgent.turnTools rather than here, since one
	// agent serves both kinds of caller.
	if !isSubAgent {

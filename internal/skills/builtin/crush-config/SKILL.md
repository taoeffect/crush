...
The crushrc `option` builtin has no key for `system_prompt_path` — set it in
`crush.json` (global or project) as shown below, or override it for a single
run with `--sys-prompt`/`-p`. (Disabling skills does have a crushrc key:
`option disable-skill <name>`, repeatable, which maps to
...
Use `.crush.json` or `crush.json` in the project root. You can also set the
same option in the workspace data config at `.crush/crush.json`, which is
loaded after project config and can override it. Relative paths are resolved
from the project/workspace directory, so `.crush/system-prompt.md` should
...
Put global config at `$XDG_CONFIG_HOME/crush/crush.json` or
`$HOME/.config/crush/crush.json`. Project config has higher priority than
global config. For a one-off run, `crush --sys-prompt /path/to/system-prompt.md`
or `crush -p /path/to/system-prompt.md` overrides configured values.

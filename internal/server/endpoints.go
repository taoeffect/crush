	...
	return []apigen.Endpoint{
		...
		apigen.Post("/v1/workspaces/{id}/agent").
			...
			Description("Validates and accepts the prompt, then dispatches the run "+
				...
				"disconnects within the reconnect grace period and is ended by the "+
				"explicit cancel endpoint, the owning client detaching for good, or "+
				"the server's maximum run duration.").
			Tags("agent").
			PathParam("id", "Workspace ID").
		...
		apigen.Post("/v1/workspaces/{id}/agent/init").
			...
			Description("Idempotent: keeps the workspace's existing agent, so a reconnecting client cannot strand running work.").
			Tags("agent").
			PathParam("id", "Workspace ID").
			Fails(400, 404, 500).
		...
		apigen.Post("/v1/workspaces/{id}/agent/runs/{rid}/cancel").
			Summary("Cancel agent run").
			Description("Cancels one run of the workspace's agent by run ID; other runs and sessions are unaffected.").
			Tags("agent").
			PathParam("id", "Workspace ID").
			PathParam("rid", "Run ID").
			Fails(404, 500).
			Handle(c.handlePostWorkspaceAgentRunCancel),
		...
		apigen.Post("/v1/workspaces/{id}/agent/sessions/{sid}/prompts/clear").
			...
			Description("Removes every queued prompt for the session and returns them, oldest to newest.").
			Tags("agent").
			PathParam("id", "Workspace ID").
			PathParam("sid", "Session ID").
			Responds(proto.ClearQueueResponse{}).
			Fails(404, 500).
			Handle(c.handlePostWorkspaceAgentSessionPromptClear),

		apigen.Post("/v1/workspaces/{id}/agent/sessions/{sid}/prompts/pop").
			Summary("Pop newest queued prompt").
			Description("Removes the newest queued message from the session's prompt queue, reporting whether one was found.").
			Tags("agent").
			PathParam("id", "Workspace ID").
			PathParam("sid", "Session ID").
			Responds(proto.PopQueuedMessageResponse{}).
			Fails(404, 500).
			Handle(c.handlePostWorkspaceAgentSessionPromptPop),
		...
		apigen.Get("/v1/workspaces/{id}/config/has").
			Summary("Check whether a config field is set").
			Tags("config").
			PathParam("id", "Workspace ID").
			QueryParam("scope", "Config scope (defaults to global).").
			RequiredQueryParam("key", "Config key").
			Responds(proto.ConfigHasFieldResponse{}).
			Fails(400, 404, 500).
			Handle(c.handleGetWorkspaceConfigHas),

		apigen.Post("/v1/workspaces/{id}/config/model/default").
			Summary("Save current model choices as defaults").
			Tags("config").
			PathParam("id", "Workspace ID").
			Fails(404, 500).
			Handle(c.handlePostWorkspaceConfigModelDefault),

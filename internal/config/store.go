...
type ConfigStore struct {
	...
	// a fresh ReadFile. Config file mutations invalidate the affected
	// scope's entry.
...
func (s *ConfigStore) SetCompactionMethod(scope Scope, method CompactionMethod) error {
	return s.update(scope, func(c *Config) map[string]any {
		...
		return map[string]any{"options.compaction_method": method}
	})
}
...
func nextRecentModels(cfg *Config, modelType SelectedModelType, model SelectedModel) ([]SelectedModel, bool) {
	return nextRecentModelsFromList(cfg.RecentModels[modelType], model)
}

func nextRecentModelsFromList(current []SelectedModel, model SelectedModel) ([]SelectedModel, bool) {

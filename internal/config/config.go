...
// SelectionWithReasoningEffort returns a run-local copy of the large
// model selection carrying the requested reasoning effort. It validates
// against the passed selection — the model the run will actually use,
// explicit or restored from the session — rather than the workspace
// default, and falls back to the workspace default only when the run has
// no selection of its own. The stored configuration is never touched, so
// the effort lives and dies with the run.
func (c *Config) SelectionWithReasoningEffort(large *SelectedModel, effort string) (*SelectedModel, error) {
	var selected SelectedModel
	if large != nil {
		selected = *large
	} else {
		var ok bool
		selected, ok = c.Models[SelectedModelTypeLarge]
		if !ok {
			return nil, errors.New("no large model selected; set one with the --model flag or 'model large'")
		}
	}
	if err := c.ValidateReasoningEffort(selected.Provider, selected.Model, effort); err != nil {
		return nil, err
	}

...
func Load(workingDir, dataDir string, debug bool) (*ConfigStore, error) {
	...
	if wsData, err := os.ReadFile(store.workspacePath); err == nil && len(wsData) > 0 {
		merged, mergeErr := loadWorkspaceOverride(cfg, wsData, workingDir, cfg.Options.DataDirectory, store.workspacePath)
		if mergeErr == nil {
			*cfg = *merged
			store.config = cfg
	...
	if resolved.LargeFallback {
		...
		} else if err := store.updatePreferredModelLocked(scope, SelectedModelTypeLarge, resolved.Large); err != nil {
	...
	if resolved.SmallFallback {
		...
		} else if err := store.updatePreferredModelLocked(scope, SelectedModelTypeSmall, resolved.Small); err != nil {
			return nil, fmt.Errorf("failed to update preferred small model: %w", err)
		}
	}
	if err := store.pruneInvalidRecentModelsLocked(); err != nil {
		return nil, fmt.Errorf("failed to prune invalid recent models: %w", err)
	}
	store.SetupAgents()
...
func loadWorkspaceOverride(cfg *Config, wsData []byte, workingDir, dataDir, path string) (*Config, error) {

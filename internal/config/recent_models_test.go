...
// TestUpdatePreferredModel_WorkspaceScopeDoesNotInheritGlobalEntries is a
...
func TestUpdatePreferredModel_WorkspaceScopeDoesNotInheritGlobalEntries(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, store.UpdatePreferredModel(ScopeWorkspace, SelectedModelTypeLarge, picked))
	...
	// In-memory recents still use the merged view for runtime UI state.
	require.Equal(t, []SelectedModel{
		{Provider: "anthropic", Model: "claude"},
		{Provider: "openai", Model: "gpt-4o"},
	}, store.Config().RecentModels[SelectedModelTypeLarge])
}
